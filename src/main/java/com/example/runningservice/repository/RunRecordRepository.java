...
public interface RunRecordRepository extends JpaRepository<RunRecordEntity, Long> {
    ...
    // 특정 기간 동안의 총 거리 구하기
    @Query("SELECT SUM(r.distance) FROM RunningRecord r WHERE r.date BETWEEN :startDate AND :endDate")
    ...
    @Query("SELECT SUM(r.time) FROM RunningRecord r WHERE r.date BETWEEN :startDate AND :endDate")
    ...
    List<RunRecordEntity> findByDateBetween(LocalDate startDate, LocalDate endDate);
}
